		...
		public static StatusChange parse(javax.xml.stream.XMLStreamReader reader) throws java.lang.Exception {
			...
			try {
				...
				while (!reader.isStartElement() && !reader.isEndElement())
					reader.next();
				log4j.debug("StatusChange.Factory.parse - Looking for Element = {}, found = {}", "Updates", reader.getName());

				if (reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type") != null) {
					java.lang.String fullTypeName = reader
							.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type");
					if (fullTypeName != null) {
						log4j.debug("StatusChange.Factory.parse - fullTypeName = {}", fullTypeName);
				...
				while (!done) {
					...
					if (reader.isEndElement()) {
						// We are done!
						log4j.debug("StatusChange.Factory.parse - Found End element, exiting while loop");
						done = true;
						break;
					}
					...
					if (QN_SITEID.equals(reader.getName())) {
						...
						if (null == content) content = Integer.toString(ToteLinkUpdateServiceHandler.getLastRunId());
