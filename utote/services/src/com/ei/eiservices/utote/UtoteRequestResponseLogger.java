...
public class UtoteRequestResponseLogger {
				...
				if (err.isParamsSpecified()) {
					...
					if (parms.isStringSpecified()) {
						...
						for (String parm : parms.getString()){
							if (0 == i++) responseLog.setErrorParameter1(parm);
							if (1 == i++) responseLog.setErrorParameter2(parm);
							if (2 == i++) responseLog.setErrorParameter3(parm);
							if (3 == i++) responseLog.setErrorParameter4(parm);
						}
					}
				}
			}
	...
	public int saveGetEventsRequest(ProgramServiceStub.GetEvents getEventsInput) {
		...
		if (getEventsInput.isEventsRequestSpecified()) {
			...
			requestLog.setSystemId(request.getSource().getSystemId());
			requestLog.setSourceId(request.getSource().getSourceId());
			if (request.isContestIdSpecified()) requestLog.setContestId(request.getContestId());
			if (request.isRacesSpecified()) requestLog.setRaces(request.getRaces());
			if (request.isTournamentIdSpecified()) requestLog.setTournamentId(request.getTournamentId());
		}
		em.persist(requestLog);
		em.getTransaction().commit();
		int id = requestLog.getIdUtoteRequestLog();
	...
	public int saveGetEventsResponse(int idUtoteRequestLog, ProgramServiceStub.GetEventsResponse eResponse, Integer idUtoteResponse) {
		...
		if (eResponse.isEventsResponseSpecified()) {
			...
			if (response.isTournamentIdSpecified()) responseLog.setEventId(response.getTournamentId());
			if (response.isContestIdSpecified()) responseLog.setEventId(response.getContestId());
			if (response.isRunIdSpecified()) responseLog.setRunId(Integer.parseInt(response.getRunId()));
			if (response.isGroupIdSpecified()) responseLog.setGroupId(response.getGroupId());
			...
			if (response.isErrorSpecified()) {
				...
				if (err.isMessageSpecified()) responseLog.setErrorMessage(err.getMessage());
				if (err.isNumberSpecified()) {
					responseLog.setErrorNumber(Integer.parseInt(err.getNumber()));
					responseLog.setIsError(true);
				}
	...
	public int saveGetEventDetailRequest(ProgramServiceStub.GetEventDetail getEventDetailInput) {
		...
		if (getEventDetailInput.isEventsRequestSpecified()) {
			...
			requestLog.setSourceId(request.getSource().getSourceId());
			if (request.isContestIdSpecified()) requestLog.setContestId(request.getContestId());
			if (request.isPoolsSpecified()) requestLog.setPools(request.getPools());
			if (request.isProgramSpecified()) requestLog.setProgram(request.getProgram());
			if (request.isRunnersSpecified()) requestLog.setRunners(request.getRunners());
			if (request.isRacesSpecified()) requestLog.setRaces(request.getRaces());
			if (request.isTournamentIdSpecified()) requestLog.setTournamentId(request.getTournamentId());
	...
	public int saveGetEventDetailResponse(int idUtoteRequestLog, ProgramServiceStub.GetEventDetailResponse eResponse, Integer idUtoteResponse) {
		...
		if (eResponse.isEventsResponseSpecified()) {
			...
			if (response.isEventDetailSpecified()) responseLog.setEventId(response.getEventDetail().getEventId());
			if (response.isTournamentIdSpecified()) responseLog.setEventId(response.getTournamentId());
			if (response.isContestIdSpecified()) responseLog.setEventId(response.getContestId());
			if (response.isRunIdSpecified()) responseLog.setRunId(Integer.parseInt(response.getRunId()));
			if (response.isGroupIdSpecified()) responseLog.setGroupId(response.getGroupId());
	...
	public int saveGetRaceResponse(int idUtoteRequestLog, ProgramServiceStub.GetRaceResponse rResponse, Integer idUtoteResponse) {
		...
		if (rResponse.isRaceResponseSpecified()) {
			...
			if (response.isEventIdSpecified()) responseLog.setEventId(response.getEventId());
			if (response.isRaceSpecified()) responseLog.setRaceId(response.getRace().getRaceId());
			if (response.isRunIdSpecified()) responseLog.setRunId(Integer.parseInt(response.getRunId()));
			if (response.isGroupIdSpecified()) responseLog.setGroupId(response.getGroupId());
			if (response.isSourceSpecified()) {
				responseLog.setSystemId(response.getSource().getSystemId());
