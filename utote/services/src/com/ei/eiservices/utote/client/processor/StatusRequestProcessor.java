...
public class StatusRequestProcessor {
	...
	private ToteStatusServiceStub.Source getSource() {
	...
	public boolean waitForToteStatusOkay() throws Exception {
	    ...
	    class ToteStatusCaller implements Runnable {
			...
			public void run() {
				...
				// Log the request
				requestLogId = (new UtoteRequestResponseLogger()).saveToteStatusRequest(getToteStatusInput);
				...
				if (null != sResponse) {
					// Log the response
					responseLogId = (new UtoteRequestResponseLogger()).saveToteStatusResponse(requestLogId, sResponse);
