...
public class ConfigurationRequestProcessor {
	...
	private ConfigurationServiceStub.Source getSource() {
	...
	private UtoteConfiguration cloneConfiguration(UtoteConfiguration uConfig, ConfigurationServiceStub.ConfigurationResponse cResponse) {
		
		if (cResponse.isAccountCardSpecified()) uConfig.setAccountCard(cResponse.getAccountCard());
		if (cResponse.isAccountOffsetSpecified()) uConfig.setAccountOffset(cResponse.getAccountOffset());
		if (cResponse.isAccountSizeSpecified()) uConfig.setAccountSize(cResponse.getAccountSize());
		if (cResponse.isAccountTimeoutSpecified()) uConfig.setAccountTimeout(cResponse.getAccountTimeout());
		...
		if (cResponse.isAssistanceSpecified()) uConfig.setAssistance(cResponse.getAssistance());
		if (cResponse.isBAMSpecified()) uConfig.setBam(cResponse.getBAM());
		if (cResponse.isBrandTimeSpecified()) uConfig.setBrandTime(cResponse.getBrandTime());
		if (cResponse.isCancelsSpecified()) uConfig.setCancels(cResponse.getCancels());
		if (cResponse.isCashReceiptsSpecified()) uConfig.setCashReceipts(cResponse.getCashReceipts());
		if (cResponse.isChannelsSpecified()) {
			uConfig.setHasChannels(true);
			// Create and populate Channel objects
		} else uConfig.setHasChannels(false);
		...
		} else uConfig.setHasCurrencies(false);
		if (cResponse.isCurrentTimeSpecified()) uConfig.setCurrentTime(cResponse.getCurrentTime().getTime());
		if (cResponse.isCustomerLogoSpecified()) uConfig.setCustomerLogo(cResponse.getCustomerLogo());
		if (cResponse.isDateFormatSpecified()) uConfig.setDateFormat(cResponse.getDateFormat());
		if (cResponse.isDatePictureSpecified()) uConfig.setDatePicture(cResponse.getDatePicture());
		if (cResponse.isDefaultEventSpecified()) uConfig.setDefaultEvent(cResponse.getDefaultEvent());
		...
		if (cResponse.isFractionalSpecified()) uConfig.setFractional(cResponse.getFractional());
		if (cResponse.isFunBetsSpecified()) uConfig.setFunBets(cResponse.getFunBets());
		if (cResponse.isFundedTimeoutSpecified()) uConfig.setFundedTimeout(cResponse.getFundedTimeout());
		if (cResponse.isGroupIdSpecified()) {
			// Preserve Group Id
		}
		if (cResponse.isInquirySpecified()) uConfig.setInquiry(cResponse.getInquiry());
		...
		if (cResponse.isLocationSpecified()) uConfig.setLocation(cResponse.getLocation());
		if (cResponse.isLongTicketSpecified()) uConfig.setLongTicket(cResponse.getLongTicket());
		if (cResponse.isManualLogOnSpecified()) uConfig.setManualLogOn(cResponse.getManualLogOn());
		if (cResponse.isMixedSpecified()) uConfig.setMixed(cResponse.getMixed());
		if (cResponse.isModeSpecified()) uConfig.setMode(cResponse.getMode().getValue());
		if (cResponse.isParlaySpecified()) uConfig.setParlay(cResponse.getParlay());
		if (cResponse.isPatronOffsetSpecified()) uConfig.setPatronOffset(cResponse.getPatronOffset());
		if (cResponse.isPatronSizeSpecified()) uConfig.setPatronSize(cResponse.getPatronSize());
		if (cResponse.isQuickSpecified()) uConfig.setQuick(cResponse.getQuick());
		if (cResponse.isRedactSpecified()) uConfig.setRedact(cResponse.getRedact());
		if (cResponse.isRunIdSpecified()) {
			// Preserve RunId
		}
		if (cResponse.isScrambleKeyPadSpecified()) uConfig.setScrambleKeyPad(cResponse.getScrambleKeyPad());
		if (cResponse.isSourceSpecified()) {
			// Preserve Source
		}
		if (cResponse.isSwipeRequiredSpecified()) uConfig.setSwipeRequired(cResponse.getSwipeRequired());
		if (cResponse.isTerminalSpecified()) uConfig.setTerminal(cResponse.getTerminal());
		if (cResponse.isTicketLogoSpecified()) uConfig.setTicketLogo(cResponse.getTicketLogo());
		if (cResponse.isTimeZoneIndexSpecified()) uConfig.setTimeZoneIndex(cResponse.getTimeZoneIndex());
		if (cResponse.isTokenSpecified()) uConfig.setToken(cResponse.getToken());
		if (cResponse.isToteVersionSpecified()) uConfig.setToteVersion(cResponse.getToteVersion());
		uConfig.setToteLinkVersion(cResponse.getToteLinkVersion());
		if (cResponse.isTransactionReceiptsSpecified()) uConfig.setTransactionReceipts(cResponse.getTransactionReceipts());
		if (cResponse.isUnfundedTimeoutSpecified()) uConfig.setUnfundedTimeout(cResponse.getUnfundedTimeout());
		if (cResponse.isVideoSpecified()) uConfig.setVideo(cResponse.getVideo());

		return uConfig;
	}

	private UtoteConfiguration persistConfiguration(ConfigurationServiceStub.GetConfigurationResponse uConfig) {
		log4j.entry(uConfig);
	...
	public UtoteConfiguration getConfiguration() {
